...
@WebApp.route('/op/domain/', methods=['GET', 'POST'])
def opdomain_info():
    """
    @note domain信息返回页面
    ...
    if IsShowPage(session["user_id"], MENU_DIC['OpDomainList']) == FALSE:

        return redirect("/")

    # 当前页面权限
    function_privilege = IsShowPage(session["user_id"], MENU_DIC['OpDomainList'], 'priv')
...
@WebApp.route('/op/domain/add/', methods=['GET', 'POST'])
def opdomain_add():
    """
    @note domain添加信息
    ...
    if request.method == 'POST':
        ...
        return redirect("/op/domain/")
    return render_template("domain/add_update.html", function_privilege=function_privilege)


@WebApp.route('/op/domain/comments/<iDomainId>', methods=['GET', 'POST'])
def opdomain_comments(iDomainId):
    """
...
@WebApp.route('/op/domain/update/', methods=['GET', 'POST'])
def opdomain_update():
    """
    @note domain 更新信息页面
    :return:
