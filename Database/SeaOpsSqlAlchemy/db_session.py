...
def SelectCommentByServer(iServerId):
    ...
    with GetSession() as db_ses:
        ...
        for rcdComment in db_ses.query(tables.CommentHistory).filter(tables.CommentHistory.server_id == iServerId):
            lstComment.append({'server_id':rcdComment.server_id, 'history':rcdComment.comment, 'update_time':rcdComment.update_time})
...
def SelectUser():
    ...
    with GetSession() as db_ses:
        for rcdUser in db_ses.query(tables.User).order_by(tables.User.create_time.desc()):
            dictUser = {"id":rcdUser.id, "name":rcdUser.name, "password":rcdUser.password, "create_time":rcdUser.create_time}
            if(0 == rcdUser.type):
...
def SelectUserByName(strUserName):
    ...
    """
    with GetSession() as db_ses:
        rcdUser = db_ses.query(tables.User.id, tables.User.name, tables.User.password, tables.User.type, tables.User.create_time).filter(tables.User.name == strUserName).first()

    if(None == rcdUser):
        logger.error("query user %s failed" % strUserName)
        return None

    dictUser = {"id":rcdUser.id, "name":rcdUser.name, "password":rcdUser.password, "type":rcdUser.type, "create_time":rcdUser.create_time}
    return dictUser
...
def SelectUserById(iUserId):
    ...
    """
    with GetSession() as db_ses:
        rcdUser = db_ses.query(tables.User.id, tables.User.name, tables.User.password).filter(tables.User.id == iUserId).first()
...
def InsertReturn(strJobId, strMinionId, strFun, strOperationId, iUserId, iProjectId, iSetId, strCurrentTime,strAction = None):
    """
    @note 添加salt job记录
...
def SelectOperation():
    ...
    with GetSession() as db_ses:
        #在数据库中查找所有操作的记录
        lstReturn = db_ses.query(tb.operation_id,tb.project_id,tb.set_id,tb.user_id,tb.start_time,tb.action).distinct().order_by(tables.SaltReturns.start_time.desc()).all()
...
def InsertSet(strName, iProjectId,userId):
    ...
    with GetSession() as db_ses:
        ...
        if(None == rcdTmp):
            logger.error("insert set failed. set name = %s, project_id = %s, create_userid = %s" % (strName, iProjectId, userId))
            return 0
        iTmp = rcdTmp.id
...
def SelectSetByProject(iProjectId):
    ...
    """
    with GetSession() as db_ses:
        lstRcdTmp = db_ses.query(tables.Set).filter(tables.Set.project_id == iProjectId).order_by(tables.Set.create_time.desc()).all()
...
def InsertAction(strName, strPath, iSetId):
    ...
    """
    with GetSession() as db_ses:
        rcdTmp = db_ses.query(tables.Action).filter(and_(tables.Action.name == strName, tables.Action.set_id == iSetId)).first()
        if(rcdTmp != None):
...
def SelectActionBySrvSet(strSetName):
    ...
    """
    with GetSession() as db_ses:
        lstRcd = db_ses.query(tables.Action).filter(tables.Action.set_id == tables.Set.id).filter(tables.Set.name == strSetName).all()
        if(None == lstRcd or 0 == len(lstRcd)):
            return None
